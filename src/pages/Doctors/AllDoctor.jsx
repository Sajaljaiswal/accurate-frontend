...
const AllDoctor = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                <tr><td colSpan="5" className="p-10 text-center text-gray-500">No doctors found</td></tr>
              ) : (
                doctors.map((doctor) => (
                  <tr key={doctor._id} className="hover:bg-blue-50 transition-colors">
                    ...
                    <td className="p-4 text-center text-gray-600">{doctor.specialization || "-"}</td>
                    <td className="p-4 text-center text-gray-600">{doctor.clinicName || "-"}</td>
                    <td className="p-4 text-center">
                      <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase ${
                        doctor.status === "TAGGED" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"
                      }`}>
            ...
            <div className="bg-blue-900 p-5 flex justify-between items-center text-white">
              <div>
                ...
                <p className="text-xs text-blue-200 mt-1">
                  Doctor: {selectedDoctor?.title} {selectedDoctor?.fullName} | {selectedDoctor?.clinicName}
                </p>
              </div>
              <button 
      ...
      <footer className="bg-blue-900 text-white text-xs py-3 text-center mt-auto">
